...
import { setLastInv, setInvoice } from "../../redux/ducks/invoiceSlice";
...
const schema = Yup.object().shape({
  ...
  expiryDate: Yup.date()
    ...
    .min(Yup.ref("invoiceDate"), "Expiry Date must be after Invoice Date"),
...
const AddAndEditInvoice = () => {
  ...
  const { invoicesList, lastInv } = useSelector((state) => state.invoices);
    ...
    try {
      ...
      if (!id) {
        formData.append(
          "invoice",
          lastInv[0]?.invoice
            ? (parseInt(lastInv[0]?.invoice, 10) + 1)
                .toString()
                .padStart(lastInv[0]?.invoice.length, "0")
            : "0001"
        );
        const object = Object.fromEntries(formData.entries());
        await addNewInvoice(object);
      } else {
        const object = Object.fromEntries(formData.entries());
        await editInvoice(object);
      }
      goBack();
    } catch (error) {
  ...
  const handleInputChange = async (event) => {
    ...
    if (selectedInvoice?.length) {
      const items = { ...selectedInvoice[0], [name]: value };
  ...
  const handleDateChange = (date, dateFieldName) => {
    if (dateFieldName === "invoiceDate") {
      setInvoiceDate(date);
      formValues.current = { ...formValues.current, invoiceDate: date };
    } else if (dateFieldName === "expiryDate") {
      setExpiryDate(date);
      formValues.current = { ...formValues.current, expiryDate: date };
    }

    const invoiceDate = formValues.current.invoiceDate;
    const expiryDate = formValues.current.expiryDate;

    schema
      .validateAt(dateFieldName, { invoiceDate, expiryDate })
      .then(() => {
  ...
  const handlePrintOrPreview = () => {
    generatePdf(formValues, "Invoices");
  ...
  const handleStockQuantity = (invoice) => {
    const list = [];
    itemsList.forEach((x) => {
      invoice.itemDetails.forEach((y) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const data = await getAddNewInvoice();
  ...
  useEffect(() => {
    if (id && invoicesList?.length && itemsList?.length) {
      const list = invoicesList?.filter((x) => x._id === id);
      const invoiceDate = dayjs(new Date(list[0]?.invoiceDate));
      const expiryDate = dayjs(new Date(list[0]?.expiryDate));
      formValues.current = { ...list[0], invoiceDate, expiryDate };
      billingAddressValues.current = {
        ...list[0],
        invoiceDate,
        expiryDate,
  ...
  return (
    <div className="tw-relative tw-mt-16 tw-min-h-screen tw-bg-white tw-select-none">
      ...
      <SubNavBar
        leftText={`${id ? "Edit invoice" : "Create new invoice"}`}
              ...
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  ...
                  sx={{
                    "& .MuiInputBase-root": {
                      height: 27,
                      border: formErrors.invoiceDate ? "1px solid #FF0000" : "",
                      fontSize: 12,
                    },
                    "& .MuiSvgIcon-root": {
                      color: "#8153E2",
