...
import { getSalesOrder, setLastSlo } from "../../redux/ducks/salesOrderSlice";
...
const schema = Yup.object().shape({
  ...
  expiryDate: Yup.date()
    ...
    .min(Yup.ref("salesOrderDate"), "Expiry Date must be after Sales Date"),
...
const AddAndEditSalesOrder = () => {
  ...
  const { salesOrderList, lastSlo } = useSelector((state) => state.salesOrders);
  ...
  const [salesOrderDate, setSalesOrderDate] = useState(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!id) {
        formData.append(
          "salesOrder",
          lastSlo[0]?.salesOrder
            ? (parseInt(lastSlo[0]?.salesOrder, 10) + 1)
                .toString()
                .padStart(lastSlo[0]?.salesOrder.length, "0")
            : "0001"
        );
        const object = Object.fromEntries(formData.entries());
        await addNewSalesOrder(object);
      } else {
        const object = Object.fromEntries(formData.entries());
        await editSalesOrder(object);
      }
      goBack();
    } catch (error) {
  ...
  const handleInputChange = async (event) => {
    ...
    if (selectedSalesOrder?.length) {
      const items = { ...selectedSalesOrder[0], [name]: value };
  ...
  const handleDateChange = (date, dateFieldName) => {
    if (dateFieldName === "salesOrderDate") {
      setSalesOrderDate(date);
      formValues.current = { ...formValues.current, salesOrderDate: date };
    } else if (dateFieldName === "expiryDate") {
      setExpiryDate(date);
      formValues.current = { ...formValues.current, expiryDate: date };
    }

    const salesOrderDate = formValues.current.salesOrderDate;
    const expiryDate = formValues.current.expiryDate;

    schema
      .validateAt(dateFieldName, { salesOrderDate, expiryDate })
      .then(() => {
  ...
  const handlePrintOrPreview = () => {
    generatePdf(formValues, "Sales order");
  ...
  const handleStockQuantity = (salesOrder) => {
    const list = [];
    itemsList.forEach((x) => {
      salesOrder.itemDetails.forEach((y) => {
  ...
  useEffect(() => {
    (async () => {
      const data = await getAddNewSalesOrder();
  ...
  useEffect(() => {
    if (id && salesOrderList?.length && itemsList?.length) {
      const list = salesOrderList?.filter((x) => x._id === id);
      const salesOrderDate = dayjs(new Date(list[0]?.salesOrderDate));
      const expiryDate = dayjs(new Date(list[0]?.expiryDate));
      formValues.current = { ...list[0], salesOrderDate, expiryDate };
      billingAddressValues.current = {
        ...list[0],
        salesOrderDate,
        expiryDate,
  ...
  return (
    <div className="tw-relative tw-mt-16 tw-min-h-screen tw-bg-white tw-select-none">
      ...
      <SubNavBar
        leftText={`${id ? "Edit sales order" : "Create new sales order"}`}
      ...
      <form className="tw-px-14 tw-py-8 tw-mt-12 tw-bg-white tw-border-b">
        ...
        <div className="tw-grid md:tw-grid-cols-11 md:tw-grid-flow-row tw-gap-10">
          <div className="tw-col-span-4">
            ...
            {/* SalesOrder_date_input */}
            <label className="tw-mb-[6px]">
              {formErrors?.salesOrderDate
                ? formErrors.salesOrderDate
                : "Sales order date"}
            </label>
            <div className="tw-pb-4">
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  ...
                  onChange={(date) => handleDateChange(date, "salesOrderDate")}
                  className="tw-w-full"
                  sx={{
                    "& .MuiInputBase-root": {
                      height: 27,
                      border: formErrors.salesOrderDate
                        ? "1px solid #FF0000"
                        : "",
                      fontSize: 12,
                    },
            ...
            <div className="tw-flex">
              <div className="tw-w-[60%] tw-pb-4">
                ...
                <input
                  ...
                  type="text"
                  placeholder="SLO-0001"
                  value={
                    id
                      ? lastSlo[0]?.salesOrder
                        ? "SLO-" +
                          parseInt(lastSlo[0]?.salesOrder, 10)
                            .toString()
                            .padStart(lastSlo[0]?.salesOrder.length, "0")
                        : "SLO-0001"
                      : lastSlo[0]?.salesOrder
...
export default AddAndEditSalesOrder;
