...
import {
  setLastPso,
  setPurchaseOrder,
} from "../../redux/ducks/purchaseOrderSlice";
...
const AddAndEditPurchaseOrder = () => {
  ...
  const { lastPso, purchaseOrdersList } = useSelector(
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!id) {
        formData.append(
          "purchaseOrder",
          lastPso[0]?.purchaseOrder
            ? (parseInt(lastPso[0]?.purchaseOrder, 10) + 1)
                .toString()
                .padStart(lastPso[0]?.purchaseOrder.length, "0")
            : "0001"
        );
        const object = Object.fromEntries(formData.entries());
        await addNewPurchaseOrder(object);
      } else {
        const object = Object.fromEntries(formData.entries());
        await editPurchaseOrder(object);
      }
      goBack();
    } catch (error) {
  ...
  const handleInputChange = async (event) => {
    ...
    if (selectedPurchaseOrder?.length) {
      const items = { ...selectedPurchaseOrder[0], [name]: value };
  ...
  const handleDateChange = (date, dateFieldName) => {
    if (dateFieldName === "purchaseOrderDate") {
      setPurchaseOrderDate(date);
      formValues.current = { ...formValues.current, purchaseOrderDate: date };
    } else if (dateFieldName === "expiryDate") {
      setExpiryDate(date);
      formValues.current = { ...formValues.current, expiryDate: date };
    }

    const purchaseOrderDate = formValues.current.purchaseOrderDate;
    const expiryDate = formValues.current.expiryDate;

    schema
      .validateAt(dateFieldName, { purchaseOrderDate, expiryDate })
      .then(() => {
  ...
  const handlePrintOrPreview = () => {
    generatePdf(formValues, "Purchase order");
  ...
  const handleStockQuantity = (purchaseOrder) => {
    const list = [];
    itemsList.forEach((x) => {
      purchaseOrder.itemDetails.forEach((y) => {
  ...
  useEffect(() => {
    (async () => {
      const purchaseOrders = await getAllPurchaseOrder();
      const data = await getAddNewPurchaseOrder();
  ...
  useEffect(() => {
    if (id && purchaseOrdersList?.length && itemsList?.length) {
      const list = purchaseOrdersList?.filter((x) => x._id === id);
      const purchaseOrderDate = dayjs(new Date(list[0]?.purchaseOrderDate));
      const expiryDate = dayjs(new Date(list[0]?.expiryDate));
      formValues.current = { ...list[0], purchaseOrderDate, expiryDate };
      billingAddressValues.current = {
        ...list[0],
        purchaseOrderDate,
        expiryDate,
  ...
  }, [id, purchaseOrdersList, itemsList]);
  ...
  return (
    <div className="tw-relative tw-mt-16 tw-min-h-screen tw-bg-white tw-select-none">
      ...
      <SubNavBar
        leftText={`${id ? "Edit purchase order" : "Create new purchase order"}`}
      ...
      <form className="tw-px-14 tw-py-8 tw-mt-12 tw-bg-white tw-border-b">
        ...
        <div className="tw-grid md:tw-grid-cols-11 md:tw-grid-flow-row tw-gap-10">
          <div className="tw-col-span-4">
            ...
            {/* purchaseOrderDate_input */}
            <label className="tw-mb-[6px]">
              {formErrors?.purchaseOrderDate
                ? formErrors.purchaseOrderDate
                : "Purchase order date"}
            </label>
            <div className="tw-pb-4">
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  ...
                  onChange={(date) =>
                    handleDateChange(date, "purchaseOrderDate")
                  }
                  className="tw-w-full"
                  sx={{
                    "& .MuiInputBase-root": {
                      height: 27,
                      border: formErrors.purchaseOrderDate
                        ? "1px solid #FF0000"
                        : "",
                      fontSize: 12,
                    },
                ...
                <input
                  ...
                  value={
                    id
                      ? lastPso[0]?.purchaseOrder
                        ? "PSO-" +
                          parseInt(lastPso[0]?.purchaseOrder, 10)
                            .toString()
                            .padStart(lastPso[0]?.purchaseOrder.length, "0")
                        : "PSO-0001"
                      : lastPso[0]?.purchaseOrder
...
export default AddAndEditPurchaseOrder;
