...
import { addNewQuote, editQuote } from "../../services/estimateServices";
import dayjs from "dayjs";
import { setLastQot, setQuote } from "../../redux/ducks/estimateSlice";
...
const AddAndEditQuote = () => {
  ...
  const { lastQot, quotesList } = useSelector((state) => state.estimates);
    ...
    try {
      ...
      if (!id) {
        formData.append(
          "quote",
          lastQot[0]?.quote
            ? (parseInt(lastQot[0]?.quote, 10) + 1)
                .toString()
                .padStart(lastQot[0]?.quote.length, "0")
            : "0001"
        );
        const object = Object.fromEntries(formData.entries());
        await addNewQuote(object);
      } else {
        const object = Object.fromEntries(formData.entries());
        await editQuote(object);
      }
      goBack();
    } catch (error) {
  ...
  const handleInputChange = async (event) => {
    ...
    if (selectedQuote?.length) {
      const items = { ...selectedQuote[0], [name]: value };
  ...
  const handleDateChange = (date, dateFieldName) => {
    if (dateFieldName === "quoteDate") {
      setQuoteDate(date);
      formValues.current = { ...formValues.current, quoteDate: date };
    } else if (dateFieldName === "expiryDate") {
      setExpiryDate(date);
      formValues.current = { ...formValues.current, expiryDate: date };
    }

    const quoteDate = formValues.current.quoteDate;
    const expiryDate = formValues.current.expiryDate;

    schema
      .validateAt(dateFieldName, { quoteDate, expiryDate })
      .then(() => {
  ...
  const handleStockQuantity = (quote) => {
    const list = [];
    itemsList.forEach((x) => {
      quote.itemDetails.forEach((y) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const data = await getAddNewQuote();
  ...
  useEffect(() => {
    if (id && quotesList?.length && itemsList?.length) {
      const list = quotesList?.filter((x) => x._id === id);
      const quoteDate = dayjs(new Date(list[0]?.quoteDate));
      const expiryDate = dayjs(new Date(list[0]?.expiryDate));
      formValues.current = { ...list[0], quoteDate, expiryDate };
      billingAddressValues.current = {
        ...list[0],
        quoteDate,
        expiryDate,
  ...
  return (
    <div className="tw-relative tw-mt-16 tw-min-h-screen tw-bg-white tw-select-none">
      ...
      <SubNavBar
        leftText={`${id ? "Edit quote" : "Create new quote"}`}
        ...
        <div className="tw-grid md:tw-grid-cols-11 md:tw-grid-flow-row tw-gap-10">
          <div className="tw-col-span-4">
            ...
            <label className="tw-mb-[6px]">
              {formErrors?.quoteDate ? formErrors.quoteDate : "Quote date"}
            </label>
            <div className="tw-pb-4">
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  value={quoteDate}
                  onChange={(date) => handleDateChange(date, "quoteDate")}
                  className="tw-w-full"
                  sx={{
                    "& .MuiInputBase-root": {
                      height: 27,
                      border: formErrors.quoteDate ? "1px solid #FF0000" : "",
                      fontSize: 12,
                    },
                    "& .MuiSvgIcon-root": {
                      color: "#8153E2",
            ...
            <div className="tw-flex">
              <div className="tw-w-[60%] tw-pb-4">
                ...
                <input
                  ...
                  type="text"
                  placeholder="QOT-0001"
                  value={
                    id
                      ? lastQot[0]?.quote
                        ? "QOT-" +
                          parseInt(lastQot[0]?.quote, 10)
                            .toString()
                            .padStart(lastQot[0]?.quote.length, "0")
                        : "QOT-0001"
                      : lastQot[0]?.quote
...
export default AddAndEditQuote;
