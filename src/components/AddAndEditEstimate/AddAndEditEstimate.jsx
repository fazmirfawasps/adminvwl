...
import { addNewEstimate, editEstimate } from "../../services/estimateServices";
import {
  getAddNewEstimate,
  getEstimate,
...
const schema = Yup.object().shape({
  ...
  expiryDate: Yup.date()
    ...
    .min(Yup.ref("estimateDate"), "Expiry Date must be after Estimate Date"),
...
const AddAndEditEstimate = () => {
  ...
  const { lastEst, estimatesList } = useSelector((state) => state.estimates);
  ...
  console.log(filteredOptions,'the optionsss');
    ...
    try {
      ...
      if (!id) {
        formData.append(
          "estimate",
          lastEst[0]?.estimate
            ? (parseInt(lastEst[0]?.estimate, 10) + 1)
                .toString()
                .padStart(lastEst[0]?.estimate.length, "0")
            : "0001"
        );
        const object = Object.fromEntries(formData.entries());
        await addNewEstimate(object);
      } else {
        const object = Object.fromEntries(formData.entries());
        await editEstimate(object);
      }
      goBack();
    } catch (error) {
  ...
  const handleInputChange = async (event) => {
    ...
    if (selectedEstimate?.length) {
      const items = { ...selectedEstimate[0], [name]: value };
  ...
  const handleDateChange = (date, dateFieldName) => {
    if (dateFieldName === "estimateDate") {
      setEstimateDate(date);
      formValues.current = { ...formValues.current, estimateDate: date };
    } else if (dateFieldName === "expiryDate") {
      setExpiryDate(date);
      formValues.current = { ...formValues.current, expiryDate: date };
    }

    const estimateDate = formValues.current.estimateDate;
    const expiryDate = formValues.current.expiryDate;

    schema
      .validateAt(dateFieldName, { estimateDate, expiryDate })
      .then(() => {
  ...
  const handlePrintOrPreview = () => {
    generatePdf(formValues, "Estimates");
  ...
  const handleStockQuantity = (estimate) => {
    const list = [];
    itemsList.forEach((x) => {
      estimate.itemDetails.forEach((y) => {
  ...
  useEffect(() => {
    if (id && estimatesList?.length && itemsList?.length) {
      const list = estimatesList?.filter((x) => x._id === id);
      const estimateDate = dayjs(new Date(list[0]?.estimateDate));
      const expiryDate = dayjs(new Date(list[0]?.expiryDate));
      formValues.current = { ...list[0], estimateDate, expiryDate };
      billingAddressValues.current = {
        ...list[0],
        estimateDate,
        expiryDate,
  ...
  return (
    <div className="tw-relative tw-mt-16 tw-min-h-screen tw-bg-white tw-select-none">
      ...
      <SubNavBar
        leftText={`${id ? "Edit estimate" : "Create new estimate"}`}
      ...
      <form className="tw-px-14 tw-py-8 tw-mt-12 tw-bg-white tw-border-b">
        ...
        <div className="tw-grid md:tw-grid-cols-11 md:tw-grid-flow-row tw-gap-10">
          <div className="tw-col-span-4">
            ...
            <div className="Estimate_date_input tw-pb-4">
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  value={estimateDate}
                  onChange={(date) => handleDateChange(date, "estimateDate")}
                  className="tw-w-full"
                  sx={{
                    "& .MuiInputBase-root": {
                      height: 27,
                ...
                <input
                  ...
                  value={
                    id
                      ? lastEst[0]?.estimate
                        ? "EST-" +
                          parseInt(lastEst[0]?.estimate, 10)
                            .toString()
                            .padStart(lastEst[0]?.estimate.length, "0")
                        : "EST-0001"
                      : lastEst[0]?.estimate
...
export default AddAndEditEstimate;
