...
import {
  addNewGoodsReceiveNote,
  editGoodsReceiveNote,
  getAddNewGoodsReceiveNote,
...
} from "../../redux/ducks/goodsReceiveNoteSlice";
...
const schema = Yup.object().shape({
  ...
  goodsReceiveNoteDate: Yup.date().required(
    "Goods receive note date is required"
  ),
  expiryDate: Yup.date()
    .required("Expiry date is required")
    .min(
      Yup.ref("goodsReceiveNoteDate"),
      "Expiry Date must be after Goods Date"
    ),
...
const AddAndEditGoodsReceiveNote = () => {
  const { id } = useParams();
  ...
  const { goodsReceiveNotesList, lastGrn } = useSelector(
  ...
  const [goodsReceiveNoteDate, setGoodsReceiveNoteDate] = useState(null);
  ...
  const [selectedGoodsReceiveNote, setSelectedGoodsReceiveNote] =
    useState(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!id) {
        formData.append(
          "goodsReceiveNote",
          lastGrn[0]?.goodsReceiveNote
            ? (parseInt(lastGrn[0]?.goodsReceiveNote, 10) + 1)
                .toString()
                .padStart(lastGrn[0]?.goodsReceiveNote.length, "0")
            : "0001"
        );
        const object = Object.fromEntries(formData.entries());
        await addNewGoodsReceiveNote(object);
      } else {
        const object = Object.fromEntries(formData.entries());
        await editGoodsReceiveNote(object);
      }
      goBack();
    } catch (error) {
  ...
  const handleInputChange = async (event) => {
    ...
    if (selectedGoodsReceiveNote?.length) {
      const items = { ...selectedGoodsReceiveNote[0], [name]: value };
  ...
  const handleDateChange = (date, dateFieldName) => {
    if (dateFieldName === "goodsReceiveNoteDate") {
      setGoodsReceiveNoteDate(date);
      formValues.current = {
        ...formValues.current,
        goodsReceiveNoteDate: date,
      };
    ...
    const goodsReceiveNoteDate = formValues.current.goodsReceiveNoteDate;
    const expiryDate = formValues.current.expiryDate;

    schema
      .validateAt(dateFieldName, { goodsReceiveNoteDate, expiryDate })
      .then(() => {
  ...
  const handlePrintOrPreview = () => {
    generatePdf(formValues, "GoodsReceiveNote");
  ...
  const handleStockQuantity = (goodsReceiveNote) => {
    const list = [];
    itemsList.forEach((x) => {
      goodsReceiveNote.itemDetails.forEach((y) => {
  ...
  useEffect(() => {
    (async () => {
      const goodsReceiveNotes = await getAllGoodsReceiveNote();
      const data = await getAddNewGoodsReceiveNote();
  ...
  useEffect(() => {
    if (id && goodsReceiveNotesList?.length && itemsList?.length) {
      const list = goodsReceiveNotesList?.filter((x) => x._id === id);
      const goodsReceiveNoteDate = dayjs(
        new Date(list[0]?.goodsReceiveNoteDate)
      );
      const expiryDate = dayjs(new Date(list[0]?.expiryDate));
      formValues.current = { ...list[0], goodsReceiveNoteDate, expiryDate };
      billingAddressValues.current = {
        ...list[0],
        goodsReceiveNoteDate,
        expiryDate,
      };
      ...
      setSelectedGoodsReceiveNote(list);
      ...
      setGoodsReceiveNoteDate(goodsReceiveNoteDate);
  ...
  }, [id, goodsReceiveNotesList, itemsList]);
  ...
  return (
    <div className="tw-relative tw-mt-16 tw-min-h-screen tw-bg-white tw-select-none">
      ...
      <SubNavBar
        leftText={`${
          id ? "Edit goods receive note" : "Create new goods receive note"
        }`}
        ...
        <div className="tw-grid md:tw-grid-cols-11 md:tw-grid-flow-row tw-gap-10">
          <div className="tw-col-span-4">
            ...
            <label className="tw-mb-[6px]">
              {formErrors?.goodsReceiveNoteDate
                ? formErrors.goodsReceiveNoteDate
                : "Good receive note date"}
            </label>
            <div className="tw-pb-4">
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  value={goodsReceiveNoteDate}
                  onChange={(date) =>
                    handleDateChange(date, "goodsReceiveNoteDate")
                  }
                  className="tw-w-full"
                  sx={{
                    "& .MuiInputBase-root": {
                      height: 27,
                      border: formErrors.goodsReceiveNoteDate
                        ? "1px solid #FF0000"
                        : "",
                      fontSize: 12,
                    },
                ...
                <input
                  ...
                  value={
                    id
                      ? lastGrn[0]?.goodsReceiveNote
                        ? "GRN-" +
                          parseInt(lastGrn[0]?.goodsReceiveNote, 10)
                            .toString()
                            .padStart(lastGrn[0]?.goodsReceiveNote.length, "0")
                        : "GRN-0001"
                      : lastGrn[0]?.goodsReceiveNote
...
export default AddAndEditGoodsReceiveNote;
