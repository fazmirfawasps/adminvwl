...
import { setLastRet, setRefundReceipt } from "../../redux/ducks/receiptSlice";
...
const schema = Yup.object().shape({
  ...
  refundReceiptDate: Yup.date().required("Refund receipt date is required"),
  expiryDate: Yup.date()
    .required("Expiry date is required")
    .min(
      Yup.ref("refundReceiptDate"),
      "Expiry Date must be after Receipt Date"
    ),
  billingAddress: Yup.string().required("Billing address is required"),
  billingAddressCity: Yup.string().required("City is required"),
  billingAddressState: Yup.string().required("State is required"),
  billingAddressPostalCode: Yup.string()
    .matches(/^[0-9]{6}$/, "Zip code must be a 6-digit number")
    .required("Zip code is required"),
  billingAddressContactNumber: Yup.string().required(
...
const initialAddress = {
  ...
  billingAddressStateIso: "",
};

const AddAndEditRefundReceipt = () => {
  const { id } = useParams();
  ...
  const { refundReceiptsList, lastRet } = useSelector(
  ...
  const [refundReceiptDate, setRefundReceiptDate] = useState(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!id) {
        formData.append(
          "refundReceipt",
          lastRet[0]?.refundReceipt
            ? (parseInt(lastRet[0]?.refundReceipt, 10) + 1)
                .toString()
                .padStart(lastRet[0]?.refundReceipt.length, "0")
            : "0001"
        );
        const object = Object.fromEntries(formData.entries());
        await addNewRefundReceipt(object);
      } else {
        const object = Object.fromEntries(formData.entries());
        await editRefundReceipt(object);
      }
      goBack();
    } catch (error) {
  ...
  const handleInputChange = async (event) => {
    ...
    if (selectedRefundReceipt?.length) {
      const items = { ...selectedRefundReceipt[0], [name]: value };
  ...
  const handleDateChange = (date, dateFieldName) => {
    if (dateFieldName === "refundReceiptDate") {
      setRefundReceiptDate(date);
      formValues.current = { ...formValues.current, refundReceiptDate: date };
    } else if (dateFieldName === "expiryDate") {
      setExpiryDate(date);
      formValues.current = { ...formValues.current, expiryDate: date };
    }

    const refundReceiptDate = formValues.current.refundReceiptDate;
    const expiryDate = formValues.current.expiryDate;

    schema
      .validateAt(dateFieldName, { refundReceiptDate, expiryDate })
      .then(() => {
  ...
  const assignSelectValue = async (customer) => {
    const {
      firstName,
      lastName,
      billingAddress,
    ...
    handleGetCities(billingAddressStateIso);
  ...
  const handleSubmitBillingAddressChange = async (e) => {
    try {
      ...
      formData.append("_id", selectedCustomer?._id);
      const object = Object.fromEntries(formData.entries());

      const data = await changeBillingAddress(object);

      if (data) {
        const list = customersList.map((x) => {
          if (x._id === data._id) {
            return data;
          }
          return x;
        ...
        formValues.current = {
          ...formValues.current,
          billingAddressContactNumber: data?.billingAddressContactNumber,
          ...
          billingAddressStateIso: data?.billingAddressStateIso,
        };
        setFormErrors({
          ...formErrors,
          billingAddress: null,
  ...
  const handleStockQuantity = (refundReceipt) => {
    const list = [];
    itemsList.forEach((x) => {
      refundReceipt.itemDetails.forEach((y) => {
    ...
    if (editAddress) {
      ...
      if (type === "state") {
        billingAddressValues.current = {
          ...billingAddressValues.current,
          billingAddressCity: "",
          billingAddressState: name,
          billingAddressStateIso: isoCode,
        };
        setFormAddress((prev) => ({
          ...prev,
          billingAddressCity: "",
  ...
  useEffect(() => {
    (async () => {
      const refundReceipt = await getAllRefundReceipts();
      const data = await getAddNewRefundReceipt();
  ...
  useEffect(() => {
    if (id && refundReceiptsList?.length && itemsList?.length) {
      const list = refundReceiptsList?.filter((x) => x._id === id);
      const refundReceiptDate = dayjs(new Date(list[0]?.refundReceiptDate));
      const expiryDate = dayjs(new Date(list[0]?.expiryDate));
      formValues.current = { ...list[0], refundReceiptDate, expiryDate };
      billingAddressValues.current = {
        ...list[0],
        refundReceiptDate,
        expiryDate,
      ...
      handleGetCities(list[0]?.billingAddressStateIso);

      setFormAddress({ ...billingAddressValues.current });
      setSelectedRefundReceipt(list);
      setItemDetails(list[0]?.itemDetails);
      setInputValue(list[0]?.customer);
      setDiscount(list[0]?.discount);
      setDiscountInputValue(list[0]?.discount);
      setRefundReceiptDate(refundReceiptDate);
      setExpiryDate(expiryDate);
      setSelectedCustomer({ _id: list[0]?.customerId });
  ...
  }, [id, refundReceiptsList, itemsList]);
  ...
  return (
    <div className="tw-relative tw-mt-16 tw-min-h-screen tw-bg-white tw-select-none">
      ...
      <SubNavBar
        leftText={`${id ? "Edit refund receipt" : "Create new refund receipt"}`}
      ...
      <form className="tw-px-14 tw-py-8 tw-mt-12 tw-bg-white tw-border-b">
        ...
        <div className="tw-grid md:tw-grid-cols-11 md:tw-grid-flow-row tw-gap-10">
          <div className="tw-col-span-4">
            ...
            <div className="tw-pb-4">
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  value={refundReceiptDate}
                  onChange={(date) =>
                    handleDateChange(date, "refundReceiptDate")
                  }
                  className="tw-w-full"
                  sx={{
                    "& .MuiInputBase-root": {
                      height: 27,
                      border: formErrors.refundReceiptDate
                        ? "1px solid #FF0000"
                        : "",
                      fontSize: 12,
                    },
          ...
          <div className="tw-col-span-7 tw-pr-14">
                ...
                <input
                  ...
                  value={
                    id
                      ? lastRet[0]?.refundReceipt
                        ? "RET-" +
                          parseInt(lastRet[0]?.refundReceipt, 10)
                            .toString()
                            .padStart(lastRet[0]?.refundReceipt.length, "0")
                        : "RET-0001"
                      : lastRet[0]?.refundReceipt
            ...
            {/* Expiry_date_input */}
            <label className="tw-mb-[6px]">
              {formErrors?.expiryDate ? formErrors.expiryDate : "Expiry date"}
            </label>
            <div className="Expiry_date_input tw-pb-[18px] tw-w-[60%]">
              <LocalizationProvider dateAdapter={AdapterDayjs}>
                <DatePicker
                  value={expiryDate}
                  ...
                  sx={{
                    "& .MuiInputBase-root": {
                      height: 27,
                      border: formErrors.expiryDate ? "1px solid #FF0000" : "",
                      fontSize: 12,
                    },
                    "& .MuiSvgIcon-root": {
                      color: "#8153E2",
            ...
            {/* last four inputs */}
            <div className="tw-flex tw-py-4 tw-mt-1.5 tw-gap-5">
              <div className="relative tw-w-3/6">
                <input
                  autoComplete="off"
                  value={
                    !editAddress
                      ? formValues?.current?.billingAddressCity
                      : billingAddressValues?.current?.billingAddressCity
                  }
                  placeholder="City"
                  onClick={() =>
              ...
              <div className="relative tw-w-3/6">
                <input
                  ...
                  value={
                    !editAddress
                      ? formValues?.current?.billingAddressState
                      : billingAddressValues?.current?.billingAddressState
                  }
                  placeholder="State"
                  onClick={() =>
            ...
            <div className="tw-flex tw-mt-1 tw-gap-5">
              <div className="tw-w-3/6">
                <Input
                  ...
                  value={
                    !editAddress
                      ? formValues?.current?.billingAddressPostalCode
                      : billingAddressValues?.current?.billingAddressPostalCode
                  }
              ...
              <div className="tw-w-3/6">
                <Input
                  value={
                    !editAddress
                      ? formValues?.current?.billingAddressContactNumber
                      : billingAddressValues?.current
                          ?.billingAddressContactNumber
                  }
                  ...
                  onChange={handleChangeBillingAddressInput}
                  errorMessage={formErrors.billingAddressContactNumber}
                  style={{
                    border: formErrors.billingAddressContactNumber
                      ? "1px solid #FF0000"
                      : "1px solid #0000003b",
                    height: "27px",
                    fontSize: "12px",
      ...
      {/* bottom section */}
      <div className="tw-z-0 tw-grid md:tw-grid-cols-2 md:tw-grid-flow-row tw-px-28 tw-bg-white tw-pb-8">
        <div className="span-col-1 tw-flex tw-justify-start tw-pb-4 md:tw-pb-0">
          <button
        ...
        <div className="span-col-1 xl:tw-pl-16 2xl:tw-pl-20">
...
export default AddAndEditRefundReceipt;
